...
- **내역 패널**: 우측 사이드 패널(폭 320px). 창모드에선 열/닫을 때 창 폭을 패널만큼 확장/축소
  (화면 폭 상한), 최대화 상태에선 목록과 공간 분할. **변경됐을 때만 다시 그림**(dirty 플래그).
  토글은 **열 때 창 확장 → 패널 pack, 닫을 때 패널 제거 → 창 축소** 순서로, 사이에 `update_idletasks`
  없이 처리해 좌측 목록 폭을 유지 → **이중 redraw 없이 한 번에** 열리고 닫힌다.
  항목 형식은 `파일명 (영상명) · 성공여부` / `일시 포맷/확장자 품질`.
...
- **[확인]**: 현재 빌드 종류에 맞는 zip을 받아 도우미 배치(.bat)를 실행하고 앱 종료.
  배치가 프로세스 종료를 기다렸다가 파일을 교체하고 재시작한다.
  - `lite`: 단일 exe 교체(`copy`)
  - `full`: 폴더 내용 덮어쓰기(`robocopy /E`, 사용자 데이터 삭제 없음)
