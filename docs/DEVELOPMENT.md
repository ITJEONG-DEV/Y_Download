...
| 7 | 배포용 단독 실행 exe | ⬜ 예정 (PyInstaller) |
...
├── src/
│   ├── downloader.py       # 핵심 로직: yt-dlp 래핑 (정보조회 + 다운로드)
...
### 다음 할 일 (우선순위 순)
- [ ] **의존성 설치 후 실제 실행 테스트** (아직 미실행 — pip install 필요)
- [ ] ffmpeg 설치 또는 bin/ 배치 후 실제 다운로드 검증
- [ ] 예외 처리 다듬기 (잘못된 URL, 지역제한, 네트워크 오류 메시지 한글화)
- [ ] 파일명 중복 시 처리(덮어쓰기/자동 번호) 정책 결정
- [ ] 다운로드 취소 버튼
- [ ] 설정 저장(마지막 저장 위치·기본 포맷 기억 → JSON)
