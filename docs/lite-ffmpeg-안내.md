# YouTubeDownloader (라이트 버전) — ffmpeg 준비 안내

이 라이트 버전(`YouTubeDownloader-lite.exe`)은 파일 크기를 줄이기 위해 **ffmpeg를 포함하지 않습니다.**
...
3. 그 `ffmpeg.exe`를 **`YouTubeDownloader-lite.exe`와 같은 폴더**에 복사합니다.
