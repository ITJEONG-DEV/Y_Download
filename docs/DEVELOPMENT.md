...
### 내역 렌더 성능
- **폭 측정 캐시**: `_clamp_text`/`_wrap_around`가 `CTkFont.measure`(개당 ~2.4ms)를 문자마다 호출해
  50행에 4.5초씩 걸리던 것을, **일반 tkinter 폰트 + 문자별 폭 캐시**(`_charw`)로 대체(개당 ~0.008ms).
- **페이지네이션**: 한 번에 `HISTORY_PAGE_SIZE`(15)개만 렌더하고 나머지는 "더 보기"로 이어 붙임
  (`_render_next_page`) → 내역이 많아도 위젯 폭주 방지.
- 남는 비용은 CTk 위젯 캔버스 그리기(행당 ~30ms)로 구조적 한계. 더 필요하면 내역 행을
  경량 tkinter 위젯으로 교체하는 방안 검토.
